...
if(isset($_POST['ac'])){
$sqla = "SELECT * FROM Game WHERE GameID = '" . $conn->real_escape_string($_POST['ac']) . "'";
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $insertSql = "INSERT INTO cart (GameID, Quantity, Price, TotalPrice) VALUES ('$GameID', $quantity, $price, $totalPrice)";
...
while ($row = $result->fetch_assoc()) {
    echo "<td>";
    echo "<table>";
    echo '<tr><td>' . '<img src="' . $row["Image"] . '" width="60%">' . '</td></tr><tr><td style="padding: 5px;">Title: ' . $row["Title"] . '</td></tr><tr><td style="padding: 5px;">Author: ' . $row["Developer"] . '</td></tr><tr><td style="padding: 5px;">Type: ' . $row["Genre"] . '</td></tr><tr><td style="padding: 5px;">RM' . $row["Price"] . '</td></tr><tr><td style="padding: 5px;">
    <form action="" method="post">
    Quantity: <input type="number" value="1" name="quantity" style="width: 20%"/><br>
    <input type="hidden" value="' . $row['GameID'] . '" name="ac"/>
...
$sql = "SELECT Game.Title, Game.Image, cart.Price, cart.Quantity, cart.TotalPrice FROM Game,cart WHERE Game.GameID = cart.GameID;";
$result = $conn->query($sql);
